...
export default function HomeModernPage() {
    ...
    const filteredFeatured = featuredUniversities
    ...
    return (
        <div className="pb-10" data-testid="home-page-root">
                                ...
                                return safeLink ? (
                                    isExternal ? (
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="group relative h-48 w-[320px] overflow-hidden rounded-2xl border border-card-border/70 shadow-sm transition-transform hover:-translate-y-1 dark:border-dark-border/70"
                                        >
                                            {cardContent}
                                        </a>
                                    ) : (
            ...
            {showUniversityArea ? (
                ...
                <div className="mb-7">
                    <h3 className="mb-3 text-lg font-bold text-text dark:text-dark-text">Featured Universities</h3>
                    {homeQuery.isFetching && !filteredFeatured.length ? (
                        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
                            {Array.from({ length: 3 }).map((_, index) => <UniversityCardSkeleton key={index} />)}
                        </div>
                    ) : filteredFeatured.length ? (
                        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
                            {filteredFeatured.map((university) => (
                                <UniversityCard
                                    key={university.id}
                        ...
                        <div className="card-flat p-5 text-sm text-text-muted dark:text-dark-text/70">No featured universities for this filter.</div>
                    )}
                </div>

                {filteredDeadlineUniversities.length ? (
                    <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3" data-testid="home-deadline-grid">
                        {filteredDeadlineUniversities.map((university) => (
                            <UniversityCard
                                key={university.id}
                                university={university}
                                config={homeSettings?.universityCardConfig}
            ...
            {showUniversityArea ? (
                ...
                {filteredUpcomingExamUniversities.length ? (
                    <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
                        {filteredUpcomingExamUniversities.map((university) => (
                            <UniversityCard
                                key={university.id}
                                university={university}
                                config={homeSettings?.universityCardConfig}
                    ...
                    <div className="space-y-3">
                        {onlineItems.map((exam) => {
                            ...
                            return (
                                <article
                                    key={exam.id}
                                    className="card-flat border border-card-border/70 p-4 dark:border-dark-border/70"
                                >
                ...
                {filteredNews.length ? (
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {filteredNews.map((item) => (
                            <article key={item._id} className="card-flat overflow-hidden border border-card-border/70 dark:border-dark-border/70">
                ...
                {filteredResources.length ? (
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {filteredResources.map((item) => {
                            ...
                            return (
                                <article key={item._id} className="card-flat border border-card-border/70 p-4 dark:border-dark-border/70">
