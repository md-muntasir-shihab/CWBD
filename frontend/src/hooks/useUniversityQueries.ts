...
export function useUniversityCategories() {
  return useQuery<UniversityCategorySummary[]>({
    queryKey: ['universityCategories'],
    queryFn: async () => {
      if (USE_MOCK) return mockGetUniversityCategories();
      const res = await getUniversityCategories();
      const d = res.data as unknown;
      if (Array.isArray(d)) return d as UniversityCategorySummary[];
      return (d as { categories?: UniversityCategorySummary[] }).categories || [];
    },
    staleTime: 60_000,
...
export function useUniversities(params: UniversityListParams) {
  ...
  return useQuery<ApiUniversity[]>({
    ...
    enabled: Boolean(category && category.toLowerCase() !== 'all'),
    queryFn: async () => {
      if (USE_MOCK) {
        const res = mockGetUniversities({ category, clusterGroup, q, sort, page, limit });
        return res.items as unknown as ApiUniversity[];
      }
...
export function useUniversityDetail(slug: string | undefined) {
  return useQuery<ApiUniversity>({
    ...
    queryFn: async () => {
      ...
      // Handle both unwrapped (new) and wrapped (legacy) shapes
