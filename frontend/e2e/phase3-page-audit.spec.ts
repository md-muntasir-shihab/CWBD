...
test.describe('Phase3 Page Audit', () => {
    test('universities list keeps category isolation and required card actions', async ({ page }) => {
        ...
        await expect(cards.first().getByText(/Application:/i)).toBeVisible();
        await expect(cards.first().getByText(/Official Site|Official N\/A/i)).toBeVisible();
        await expect(cards.first().getByText(/Quick Apply|Apply N\/A/i)).toBeVisible();
    ...
    test('university detail shows admission structure with actionable links', async ({ page }) => {
        ...
        await expect(page.getByText(/Total Seats|Seat Distribution/i).first()).toBeVisible();
    ...
    test('resources and contact pages have usable states and submit path', async ({ page }) => {
        ...
        await page.getByLabel(/Email Address/i).fill(`phase3-${Date.now()}@campusway.local`);
        await page.getByLabel(/Subject/i).selectOption({ label: 'Technical Issue' });
