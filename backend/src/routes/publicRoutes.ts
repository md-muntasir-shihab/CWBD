...
import { login, getMe, changePassword, register, refresh, logout, verifyEmail, forgotPassword, resetPassword, verify2fa, resendOtp, checkSession, sessionStream } from '../controllers/authController';
...
router.post('/exams/:id/start', authenticate, startExam);
router.put('/exams/:id/autosave', authenticate, autosaveExam);
router.post('/exams/:id/submit', authenticate, submitExam);
...
router.post('/exams/:examId/attempt/:attemptId/submit', authenticate, submitExamAttempt);
...
router.post('/exams/upload-written-answer', authenticate, uploadMiddleware.single('file'), uploadMedia);
