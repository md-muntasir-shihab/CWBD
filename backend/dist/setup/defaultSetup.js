...
exports.runDefaultSetup = runDefaultSetup;
...
const INFO_FILE = path_1.default.resolve(process.cwd(), 'INITIAL_ACCESS_INFO.txt');
...
function generatePassword() {
    ...
    return Array.from({ length: 16 }, () => chars[crypto_1.default.randomInt(0, chars.length)]).join('');
}
async function runDefaultSetup() {
    ...
    if (fs_1.default.existsSync(INFO_FILE)) {
    ...
    const existingAdmin = await User_1.default.findOne({ role: 'superadmin' });
    if (!existingAdmin) {
    ...
    const existingStudent = await User_1.default.findOne({ username: studentUsername });
    if (!existingStudent) {
        studentPassword = generatePassword();
        const hashed = await bcryptjs_1.default.hash(studentPassword, 12);
    ...
    const existingExam = await Exam_1.default.findOne({ title: demoExamName });
    if (!existingExam) {
        const now = new Date();
        ...
        // 5 sample questions
        const sampleQs = [
            { question: 'What is the capital city of Bangladesh?', optionA: 'Chittagong', optionB: 'Dhaka', optionC: 'Rajshahi', optionD: 'Khulna', correctAnswer: 'B', explanation: 'Dhaka is the capital and largest city of Bangladesh.', marks: 1, subject: 'GK', difficulty: 'easy', order: 1 },
            { question: 'In which year was Bangladesh liberated?', optionA: '1947', optionB: '1952', optionC: '1969', optionD: '1971', correctAnswer: 'D', explanation: 'Bangladesh gained independence on December 16, 1971.', marks: 1, subject: 'GK', difficulty: 'easy', order: 2 },
            { question: 'Which river is known as the "Padma" in Bangladesh?', optionA: 'Brahmaputra', optionB: 'Meghna', optionC: 'Ganges', optionD: 'Surma', correctAnswer: 'C', explanation: 'The Ganges is called Padma after it enters Bangladesh.', marks: 1, subject: 'GK', difficulty: 'easy', order: 3 },
            { question: 'What is 25% of 400?', optionA: '50', optionB: '75', optionC: '100', optionD: '125', correctAnswer: 'C', explanation: '25% of 400 = (25/100) × 400 = 100.', marks: 1, subject: 'Math', difficulty: 'easy', order: 4 },
            { question: 'Who wrote the national anthem of Bangladesh?', optionA: 'Kazi Nazrul Islam', optionB: 'Rabindranath Tagore', optionC: 'Jasimuddin', optionD: 'Shamsur Rahman', correctAnswer: 'B', explanation: '"Amar Shonar Bangla" was written by Rabindranath Tagore.', marks: 1, subject: 'GK', difficulty: 'easy', order: 5 },
        ];
        for (const q of sampleQs) {
            await Question_1.default.create({ ...q, exam: exam._id });
        }
...
====================================================
`;
    fs_1.default.writeFileSync(INFO_FILE, content.trimStart(), 'utf8');
