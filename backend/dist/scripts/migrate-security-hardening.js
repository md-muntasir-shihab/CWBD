...
async function run() {
    ...
    await Settings_1.default.updateOne({}, {
        $set: {
    ...
    const twoFaFixResult = await User_1.default.updateMany({
    ...
    const duplicateUsers = await ActiveSession_1.default.aggregate([
        { $match: { status: 'active' } },
    ...
    for (const entry of duplicateUsers) {
        const sessions = await ActiveSession_1.default.find({ _id: { $in: entry.ids } })
        ...
        if (!terminateIds.length)
            continue;
        const result = await ActiveSession_1.default.updateMany({ _id: { $in: terminateIds }, status: 'active' }, {
            $set: {
                status: 'terminated',
...
//# sourceMappingURL=migrate-security-hardening.js.map