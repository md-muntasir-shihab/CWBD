...
async function seedContentPipeline(options = {}) {
    ...
    const secondaryStudent = students[1] || students[0];
    ...
    const examDocs = await Promise.all(examSeeds.map(async (seed) => Exam_1.default.findOneAndUpdate({ title: seed.title }, {
        $set: {
            title: seed.title,
