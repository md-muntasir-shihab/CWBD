...
async function reconcile() {
    ...
    // ──────────────────────────────────────────────
    const profilesCursor = StudentProfile_1.default.find({ groupIds: { $exists: true, $ne: [] } })
    ...
    for await (const profile of profilesCursor) {
        ...
        for (const gid of profile.groupIds ?? []) {
            const exists = await GroupMembership_1.default.findOne({
                groupId: gid,
    ...
    // ──────────────────────────────────────────────
    const groupsCursor = StudentGroup_1.default.find({
        type: 'manual',
    ...
    for await (const group of groupsCursor) {
        ...
        for (const sid of group.manualStudents ?? []) {
            ...
            const updated = await StudentProfile_1.default.updateOne({ user_id: sid, groupIds: { $ne: group._id } }, { $addToSet: { groupIds: group._id } });
            if (updated.modifiedCount > 0)
                stats.profileGroupIdsAdded++;
        }
    }
    ...
    // ──────────────────────────────────────────────
    const membershipCursor = GroupMembership_1.default.find({ membershipStatus: 'active' })
        .select('groupId studentId')
        .lean()
        .cursor();
    for await (const m of membershipCursor) {
        const updated = await StudentProfile_1.default.updateOne({ user_id: m.studentId, groupIds: { $ne: m.groupId } }, { $addToSet: { groupIds: m.groupId } });
        if (updated.modifiedCount > 0)
            stats.profileGroupIdsAdded++;
    }
    ...
    // ──────────────────────────────────────────────
    const activeGroupIds = (await StudentGroup_1.default.find({ isActive: true }).select('_id').lean()).map(g => g._id);
    const archiveResult = await GroupMembership_1.default.updateMany({ membershipStatus: 'active', groupId: { $nin: activeGroupIds } }, { $set: { membershipStatus: 'archived', removedAtUTC: new Date(), note: 'Group inactive/deleted — archived by reconciliation' } });
    ...
    // ──────────────────────────────────────────────
    const allGroups = await StudentGroup_1.default.find({ isActive: true }).select('_id memberCountCached').lean();
    for (const g of allGroups) {
        const liveCount = await GroupMembership_1.default.countDocuments({ groupId: g._id, membershipStatus: 'active' });
        if (liveCount !== g.memberCountCached) {
            await StudentGroup_1.default.updateOne({ _id: g._id }, { $set: { memberCountCached: liveCount, studentCount: liveCount } });
...
//# sourceMappingURL=migrate-group-membership-reconcile.js.map