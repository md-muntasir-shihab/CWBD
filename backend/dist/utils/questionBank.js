...
function normalizeOptionsLocalized(payload) {
    ...
    const legacy = ['A', 'B', 'C', 'D']
        .map((key) => {
        const en = String(payload[`option${key}`] ||
            payload[`option_${key.toLowerCase()}`] ||
            '').trim();
        const bn = String(payload[`option${key}_bn`] ||
...
function normalizeQuestionPayload(payload, fallbackStatus = 'draft') {
    ...
    const optionsLocalized = optionsLocalizedFromPayload.length > 0
    ...
    const explanationTextLocalized = {
        en: explanationEn,
        bn: explanationBn,
    };
    ...
    const status = statusRaw === 'pending_review' || statusRaw === 'approved' || statusRaw === 'rejected' || statusRaw === 'archived'
        ? statusRaw
        : 'draft';
...
function detectSimilarQuestions(incoming, existingRows, threshold = 0.82) {
    const matches = [];
...
async function validateImageUrl(rawUrl, maxBytes = 5 * 1024 * 1024) {
