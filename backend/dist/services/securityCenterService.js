...
function normalizeRiskyActions(value, fallback) {
    ...
    const filtered = value
        ...
        .filter((item) => RISKY_ACTION_KEYS.includes(item));
...
function normalizeFromDocument(doc) {
    ...
    const passwordPolicy = (payload.passwordPolicy || {});
    const loginProtection = (payload.loginProtection || {});
    const session = (payload.session || {});
    const adminAccess = (payload.adminAccess || {});
    const siteAccess = (payload.siteAccess || {});
    const examProtection = (payload.examProtection || {});
    const logging = (payload.logging || {});
    const twoPersonApproval = (payload.twoPersonApproval || {});
...
async function mirrorToLegacySiteSettings(settings, updatedBy) {
    ...
    await Settings_1.default.findOneAndUpdate({}, { $set: setDoc }, { upsert: true, setDefaultsOnInsert: true });
}
