...
exports.getPublicAnalyticsSettings = getPublicAnalyticsSettings;
exports.adminGetAnalyticsSettings = adminGetAnalyticsSettings;
exports.adminUpdateAnalyticsSettings = adminUpdateAnalyticsSettings;
exports.adminGetAnalyticsOverview = adminGetAnalyticsOverview;
exports.adminExportEventLogs = adminExportEventLogs;
const mongoose_1 = __importDefault(require("mongoose"));
const exceljs_1 = __importDefault(require("exceljs"));
const EventLog_1 = __importDefault(require("../models/EventLog"));
const Settings_1 = __importDefault(require("../models/Settings"));
const EVENT_TO_TOGGLE = {
...
function toNumber(value, fallback, min = 0, max = 10000) {
...
function inferModule(eventName) {
    if (eventName.startsWith('university_'))
        return 'universities';
    if (eventName.startsWith('news_'))
        return 'news';
    if (eventName.startsWith('resource_'))
        return 'resources';
    if (eventName.startsWith('exam_'))
        return 'exams';
    if (eventName.startsWith('subscription_'))
        return 'subscription';
    if (eventName.startsWith('support_'))
        return 'support';
    return 'general';
}
...
async function readAnalyticsSettings() {
    ...
    const settings = await Settings_1.default.findOne().select('analyticsSettings').lean();
    const raw = settings?.analyticsSettings || {};
...
async function trackEvent(req, res) {
    try {
        ...
        const userId = req.user?._id && mongoose_1.default.Types.ObjectId.isValid(req.user._id)
        ...
        const entry = await EventLog_1.default.create({
            userId,
...
async function getPublicAnalyticsSettings(_req, res) {
    try {
        const settings = await readAnalyticsSettings();
...
async function adminGetAnalyticsSettings(_req, res) {
    try {
        const settings = await readAnalyticsSettings();
        res.json({ settings });
    }
...
async function adminUpdateAnalyticsSettings(req, res) {
    try {
        ...
        await Settings_1.default.findOneAndUpdate({}, { $set: { analyticsSettings: next, updatedBy: req.user?._id } }, { upsert: true, new: true, setDefaultsOnInsert: true });
...
async function adminGetAnalyticsOverview(req, res) {
    try {
        ...
        const [topEvents, dailySeries, totals, funnel] = await Promise.all([
            EventLog_1.default.aggregate([{ $match: match }, { $group: { _id: '$eventName', count: { $sum: 1 } } }, { $sort: { count: -1 } }, { $limit: 20 }]),
            EventLog_1.default.aggregate([{ $match: match }, { $group: { _id: { $dateToString: { format: '%Y-%m-%d', date: '$createdAt' } }, count: { $sum: 1 } } }, { $sort: { _id: 1 } }]),
            EventLog_1.default.aggregate([
                { $match: match },
            ...
            EventLog_1.default.aggregate([{ $match: { ...match, eventName: { $in: ['exam_viewed', 'exam_started', 'exam_submitted'] } } }, { $group: { _id: '$eventName', count: { $sum: 1 } } }]),
        ]);
...
async function adminExportEventLogs(req, res) {
    try {
        const query = req.query;
        ...
        if (moduleFilter && moduleFilter !== 'all')
            match.module = moduleFilter;
        const rows = await EventLog_1.default.find(match).sort({ createdAt: -1 }).limit(10000).lean();
        if (format === 'xlsx') {
