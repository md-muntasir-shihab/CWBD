...
exports.searchSimilarQuestions = searchSimilarQuestions;
exports.revertQuestionRevision = revertQuestionRevision;
exports.bulkImportQuestions = bulkImportQuestions;
exports.getQuestionImportJob = getQuestionImportJob;
exports.exportQuestions = exportQuestions;
exports.signQuestionMediaUpload = signQuestionMediaUpload;
exports.createQuestionMedia = createQuestionMedia;
exports.getQbankPicker = getQbankPicker;
exports.incrementQbankUsage = incrementQbankUsage;
...
function toSafeObject(input) {
    if (!input || typeof input !== 'object' || Array.isArray(input))
        return {};
    return input;
}
function buildLegacyCompatibilityPayload(normalized) {
    return {
        ...normalized,
...
function buildLocalizedCompatibilityPayload(normalized) {
...
function questionPermissionFromToken(req, action) {
    ...
    if (roleAllow[action]?.includes(role)) {
        ...
        if (action === 'create' || action === 'edit' || action === 'bulk_import' || action === 'lock') {
            if (typeof perms.canEditExams === 'boolean')
                return Boolean(perms.canEditExams);
        }
        if (action === 'delete') {
            if (typeof perms.canDeleteData === 'boolean')
                return Boolean(perms.canDeleteData);
        }
        if (action === 'export') {
            if (typeof perms.canViewReports === 'boolean')
...
async function createAudit(req, action, targetId, details) {
    if (!req.user?._id || !mongoose_1.default.Types.ObjectId.isValid(req.user._id))
        return;
    ...
    await AuditLog_1.default.create({
        ...
        action,
        target_id: normalizedTargetId && mongoose_1.default.Types.ObjectId.isValid(normalizedTargetId) ? normalizedTargetId : undefined,
        target_type: 'question_bank',
        ip_address: req.ip,
        details: details || {},
    });
}
async function createQuestionRevision(questionId, snapshot, changedBy) {
    const latest = await QuestionRevision_1.default.findOne({ questionId }).sort({ revisionNo: -1 }).lean();
    const nextRevisionNo = Number(latest?.revisionNo || 0) + 1;
    const revision = await QuestionRevision_1.default.create({
        questionId,
        revisionNo: nextRevisionNo,
        snapshot,
        changedBy: changedBy && mongoose_1.default.Types.ObjectId.isValid(changedBy) ? changedBy : undefined,
        changedAt: new Date(),
    });
    return revision._id;
}
async function notifyQBankWebhook(eventType, payload) {
...
function buildFilter(query) {
    const filter = {};
    if (query.subject)
        filter.subject = String(query.subject).trim();
    if (query.chapter)
        filter.chapter = String(query.chapter).trim();
    if (query.difficulty)
        filter.difficulty = String(query.difficulty).trim().toLowerCase();
    if (query.status)
        filter.status = String(query.status).trim().toLowerCase();
    if (query.class_level || query.class)
        filter.class_level = String(query.class_level || query.class).trim();
    if (query.department)
        filter.department = String(query.department).trim();
    if (query.created_by && mongoose_1.default.Types.ObjectId.isValid(String(query.created_by))) {
    ...
    if (Number.isFinite(minScore) || Number.isFinite(maxScore)) {
        ...
        if (Number.isFinite(minScore))
            filter.quality_score.$gte = minScore;
        if (Number.isFinite(maxScore))
            filter.quality_score.$lte = maxScore;
    }
    return filter;
}
async function computeFacets(baseFilter) {
    const [subjects, chapters, tags, statuses, difficulty, qualityRange] = await Promise.all([
        Question_1.default.distinct('subject', baseFilter),
        Question_1.default.distinct('chapter', baseFilter),
        Question_1.default.distinct('tags', baseFilter),
        Question_1.default.distinct('status', baseFilter),
        Question_1.default.distinct('difficulty', baseFilter),
...
async function parseImportRows(req) {
    ...
    if (Array.isArray(payload.rows)) {
        ...
        const mapping = toSafeObject(payload.mapping);
...
async function isQuestionUsedInPublishedExam(question) {
    if (!question?.exam || !mongoose_1.default.Types.ObjectId.isValid(String(question.exam)))
        return false;
    const exam = await Exam_1.default.findById(String(question.exam)).select('isPublished status').lean();
...
async function getQuestions(req, res) {
    try {
        ...
        const [questions, total, facets] = await Promise.all([
            Question_1.default.find(filter).sort(sort).skip(skip).limit(limit).lean(),
...
async function getQuestionById(req, res) {
    try {
        const question = await Question_1.default.findById(req.params.id).lean();
        if (!question) {
            res.status(404).json({ message: 'Question not found' });
            return;
        }
        const revisions = await QuestionRevision_1.default.find({ questionId: req.params.id })
            .sort({ revisionNo: -1 })
            .limit(20)
            .lean();
...
async function createQuestion(req, res) {
    try {
        ...
        const { normalized, errors } = (0, questionBank_1.normalizeQuestionPayload)(payload, fallbackStatus);
        if (payload.question_html) {
            normalized.question_html = (0, questionBank_1.sanitizeRichHtml)(payload.question_html);
        }
        if (errors.length > 0) {
            res.status(400).json({ message: errors[0], errors });
            return;
        }
        const recentCandidates = await Question_1.default.find({ status: { $ne: 'archived' } })
        ...
        const duplicateMatches = (0, questionBank_1.detectSimilarQuestions)({ question: normalized.question, options: normalized.options }, recentCandidates, Number.isFinite(threshold) ? threshold : DEFAULT_SIMILARITY_THRESHOLD);
        ...
        const questionData = {
            ...
            flagged_duplicate: flaggedDuplicate,
            duplicate_of_ids: duplicateMatches.slice(0, 5).map((match) => new mongoose_1.default.Types.ObjectId(match.questionId)),
            revision_no: 1,
            created_by: req.user?._id,
            last_edited_by: req.user?._id,
        };
        const newQuestion = await Question_1.default.create(questionData);
        const revisionId = await createQuestionRevision(String(newQuestion._id), newQuestion.toObject(), String(req.user?._id || ''));
        await Question_1.default.findByIdAndUpdate(newQuestion._id, { previous_revision_id: revisionId, revision_no: 1 });
        ...
        res.status(201).json({
            ...
            warning,
            question: await Question_1.default.findById(newQuestion._id).lean(),
            duplicateMatches,
        });
    }
    catch (err) {
...
async function updateQuestion(req, res) {
    try {
        ...
        const existing = await Question_1.default.findById(req.params.id);
        if (!existing) {
            res.status(404).json({ message: 'Question not found' });
            return;
        }
        ...
        const { normalized, errors } = (0, questionBank_1.normalizeQuestionPayload)(payload, existing.status || 'draft');
        if (errors.length > 0) {
            res.status(400).json({ message: errors[0], errors });
            return;
        }
        const recentCandidates = await Question_1.default.find({ _id: { $ne: existing._id }, status: { $ne: 'archived' } })
            .select('_id question question_text options optionA optionB optionC optionD')
            .sort({ updatedAt: -1 })
            .limit(300)
            .lean();
        ...
        const priorSnapshot = existing.toObject();
        const previousRevisionId = await createQuestionRevision(String(existing._id), priorSnapshot, String(req.user?._id || ''));
        existing.set({
            ...buildLegacyCompatibilityPayload(normalized),
            ...buildLocalizedCompatibilityPayload(normalized),
            quality_score: quality.score,
        ...
        res.json({
            ...
            warning,
            question: await Question_1.default.findById(existing._id).lean(),
            duplicateMatches,
        });
    }
    catch (err) {
...
async function deleteQuestion(req, res) {
    try {
        ...
        existing.archived_by = req.user?._id && mongoose_1.default.Types.ObjectId.isValid(req.user._id)
...
async function approveQuestion(req, res) {
    try {
        ...
        question.moderated_by = req.user?._id && mongoose_1.default.Types.ObjectId.isValid(req.user._id)
            ? new mongoose_1.default.Types.ObjectId(req.user._id)
            : null;
        ...
        const revisionId = await createQuestionRevision(String(question._id), snapshot, String(req.user?._id || ''));
...
async function lockQuestion(req, res) {
    try {
        ...
        question.locked_by = req.user?._id && mongoose_1.default.Types.ObjectId.isValid(req.user._id)
            ? new mongoose_1.default.Types.ObjectId(req.user._id)
            : null;
        await question.save();
...
async function searchSimilarQuestions(req, res) {
    try {
        const payload = toSafeObject(req.body);
        const { normalized, errors } = (0, questionBank_1.normalizeQuestionPayload)(payload, 'draft');
        ...
        if (payload.excludeId && mongoose_1.default.Types.ObjectId.isValid(String(payload.excludeId))) {
            filter._id = { $ne: new mongoose_1.default.Types.ObjectId(String(payload.excludeId)) };
        }
        const candidates = await Question_1.default.find(filter)
            .select('_id question question_text options optionA optionB optionC optionD subject chapter quality_score status')
            .sort({ updatedAt: -1 })
            .limit(400)
            .lean();
        const matches = (0, questionBank_1.detectSimilarQuestions)({ question: normalized.question, options: normalized.options }, candidates, Number.isFinite(threshold) ? threshold : DEFAULT_SIMILARITY_THRESHOLD);
        res.json({
...
async function revertQuestionRevision(req, res) {
    try {
        if (!questionPermissionFromToken(req, 'edit')) {
            res.status(403).json({ message: 'Permission denied: question:edit' });
            return;
        ...
        const revision = await QuestionRevision_1.default.findOne({
        ...
        const currentSnapshot = question.toObject();
        const previousRevisionId = await createQuestionRevision(String(question._id), currentSnapshot, String(req.user?._id || ''));
...
async function bulkImportQuestions(req, res) {
    try {
        ...
        const importJob = await QuestionImportJob_1.default.create({
            status: 'processing',
            sourceFileName,
        ...
        let failedRows = 0;
        let duplicateRows = 0;
        const candidateCache = await Question_1.default.find({ status: { $ne: 'archived' } })
            .select('_id question question_text options optionA optionB optionC optionD')
            .sort({ updatedAt: -1 })
            .limit(600)
            .lean();
        ...
        for (let index = 0; index < rows.length; index += 1) {
            ...
            try {
                const { normalized, errors } = (0, questionBank_1.normalizeQuestionPayload)(row, defaultStatus);
                if (errors.length > 0) {
                    failedRows += 1;
                    rowErrors.push({
                        rowNumber,
                ...
                if (imageUrl) {
                    const validation = await (0, questionBank_1.validateImageUrl)(imageUrl);
                    if (!validation.ok) {
                        failedRows += 1;
                        rowErrors.push({
                            rowNumber,
                    ...
                    const media = await QuestionMedia_1.default.create({
                ...
                const duplicateMatches = (0, questionBank_1.detectSimilarQuestions)({ question: normalized.question, options: normalized.options }, [
                    ...candidateCache,
                    ...syntheticCandidates,
                ], duplicateThreshold);
                const flaggedDuplicate = duplicateMatches.length > 0;
                ...
                const questionPayload = {
                    ...
                    quality_flags: quality.flags,
                    flagged_duplicate: flaggedDuplicate,
                    duplicate_of_ids: duplicateMatches.slice(0, 5).map((entry) => new mongoose_1.default.Types.ObjectId(entry.questionId)),
                    status: defaultStatus,
                    created_by: req.user?._id,
                    last_edited_by: req.user?._id,
                    revision_no: 1,
                };
                const created = await Question_1.default.create(questionPayload);
                const revisionId = await createQuestionRevision(String(created._id), created.toObject(), String(req.user?._id || ''));
...
async function getQuestionImportJob(req, res) {
    try {
        const job = await QuestionImportJob_1.default.findById(req.params.jobId).lean();
        if (!job) {
...
async function exportQuestions(req, res) {
    try {
        ...
        const rows = await Question_1.default.find(queryFilter)
            .sort({ updatedAt: -1 })
            .lean();
        ...
        if (format === 'csv') {
            const sheet = xlsx_1.default.utils.json_to_sheet(exportRows);
        ...
        const sheet = xlsx_1.default.utils.json_to_sheet(exportRows);
        xlsx_1.default.utils.book_append_sheet(workbook, sheet, 'QBank');
...
async function signQuestionMediaUpload(req, res) {
    try {
        if (!questionPermissionFromToken(req, 'edit')) {
            res.status(403).json({ message: 'Permission denied: question:edit' });
            return;
...
async function createQuestionMedia(req, res) {
    try {
        if (!questionPermissionFromToken(req, 'edit')) {
            res.status(403).json({ message: 'Permission denied: question:edit' });
            return;
        ...
        const validation = await (0, questionBank_1.validateImageUrl)(url);
...
async function getQbankPicker(req, res) {
    try {
        ...
        if (query.class_level)
            filter.class_level = String(query.class_level).trim();
        if (query.department)
            filter.department = String(query.department).trim();
        const tags = parseCsvList(query.tags);
        if (tags.length > 0)
        ...
        let docs = await Question_1.default.find(filter)
...
async function incrementQbankUsage(req, res) {
    try {
        const payload = toSafeObject(req.body);
        const examId = payload.examId && mongoose_1.default.Types.ObjectId.isValid(String(payload.examId))
            ? new mongoose_1.default.Types.ObjectId(String(payload.examId))
            : null;
        ...
        const normalizedEntries = entries
            ...
            .filter((entry) => mongoose_1.default.Types.ObjectId.isValid(entry.questionId));
        if (normalizedEntries.length === 0) {
            res.status(400).json({ message: 'Invalid question ids' });
            return;
        }
        const updates = await Promise.all(normalizedEntries.map(async (entry) => {
            const question = await Question_1.default.findById(entry.questionId);
            if (!question)
                return null;
            ...
            const quality = (0, questionBank_1.computeQualityScore)({
                ...(0, questionBank_1.normalizeQuestionPayload)(question.toObject(), question.status || 'draft').normalized,
