...
exports.readNotificationAutomationSettings = readNotificationAutomationSettings;
exports.adminGetNotificationAutomationSettings = adminGetNotificationAutomationSettings;
exports.adminUpdateNotificationAutomationSettings = adminUpdateNotificationAutomationSettings;
...
function toNumber(value, fallback, min = 0, max = 1000) {
...
async function readNotificationAutomationSettings() {
    const defaults = defaultSettings();
    const settings = await Settings_1.default.findOne().select('notificationAutomation').lean();
    const raw = settings?.notificationAutomation || {};
    return {
        examStartsSoon: {
        ...
        templates: {
            languageMode: ['bn', 'en', 'mixed'].includes(String(raw?.templates?.languageMode || '').trim())
                ? String(raw.templates.languageMode).trim()
...
async function adminGetNotificationAutomationSettings(_req, res) {
    try {
...
async function adminUpdateNotificationAutomationSettings(req, res) {
    try {
        ...
        const next = {
            ...
            templates: {
                languageMode: ['bn', 'en', 'mixed'].includes(String(body?.templates?.languageMode || '').trim())
                    ? String(body.templates.languageMode).trim()
        ...
        await Settings_1.default.findOneAndUpdate({}, { $set: { notificationAutomation: next, updatedBy: req.user?._id } }, { upsert: true, new: true, setDefaultsOnInsert: true });
...
//# sourceMappingURL=notificationAutomationController.js.map